...
function loaded() {
    ...
    .then(function(data){
        ...
        for(let kicsoda = 0; kicsoda < data.length; kicsoda++) {
            if (data[kicsoda].name == window.location.href.split('#')[1]) {
                szamakie = kicsoda
                break
            }
        }
        ...
        if (szamakie != null && data[szamakie] != null) {
            data = data[szamakie]
        }
...
function deleteSeriesFunc(confirmed) {
    if (!confirmed) {
        edit()
...
function addSeasonToSeriesFunc(confirmed) {
    if (!confirmed) {
        edit()
...
function addEpisodeToSeasonFunc(confirmed) {
    if (!confirmed) {
        edit()
...
function borito_valtoztatasFunc(confirmed) {
    if (!confirmed) {
        edit()
...
function setWatched(kinek, files, evad, eps) {
    fetch("./watched.json")
    ...
    .then(function(data){
        ...
        var hanyadik = undefined
        for (let ember = 0; ember <= data.users.length; ember++) {
            if (data.users[ember].name == kinek) {
                hanyadik = ember
                break
            }
        }
        ...
        for (let sori = 0; sori <= data.users[hanyadik].sorozatok.length; sori++) {
            if (data.users[hanyadik].sorozatok[sori].files == files) {
                sorozatszam = sori
                break
            }
        }
        ...
        for (let evadoscucc = 0; evadoscucc <= data.users[hanyadik].sorozatok[sorozatszam].watched.length; evadoscucc++) {
            if (data.users[hanyadik].sorozatok[sorozatszam].watched[evadoscucc].evad == evad) {
        ...
        var masszivdata = data.users[hanyadik].sorozatok[sorozatszam].watched[hanyadikevados].epizodok
